...
foreach ($mois as $m => $jours) {
    echo "<tr><td>$m</td><td>$jours</td></tr>";     //parcourir
}

echo "</table>";

// Tri du tableau par nombre de jours
uasort($mois, function ($a, $b) {           //Tri
    return $a - $b;
});
...
foreach ($mois as $m => $jours) {
    echo "<tr><td>$m</td><td>$jours</td></tr>";     //parcourir  tri
}
...
foreach ($capitales as $capitale => $pays) {    //analyse
    if (substr($capitale, 0, 1) !== 'B') {      //Substr = extract
        unset($capitales[$capitale]);           //Unset = delete
    }
}
